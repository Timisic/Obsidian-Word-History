...
interface WordHistorySettings {
  outputPath: string;
  updateMode: "manual" | "interval" | "git-changes";
  intervalDays: number;
  lastRunAt: number;
  lastSeenHead: string;
}

const DEFAULT_SETTINGS: WordHistorySettings = {
...
function renderChartSvg(analysis: any, width?: number) {
...
function gitText(repoPath: string, args: string[]): Promise<string> {
  return gitBuffer(repoPath, args).then((buffer) => buffer.toString("utf8"));
}

function gitBuffer(repoPath: string, args: string[]): Promise<Buffer> {
